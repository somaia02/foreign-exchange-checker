...
export default function Converter() {
  ...
  let receiveValue: number | null;
  if (data.error !== "") return <p>{data.error}</p>;
  if (data.loading) return <p>Loading ... </p>;
  receiveValue = sendValue ? Number((sendValue * data.rate!).toFixed(2)) : null;
...
function CalculatorItem({ title, currency, value, onCurrencyChange, onValueChange, disabled=[]}: CalculatorItemProps) {
